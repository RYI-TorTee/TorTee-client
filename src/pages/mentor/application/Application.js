...
export default function MentorApplication() {
    ...
    const fetchApplicationApi = () => {
        axiosInstance.get(`${RYI_URL}/mentor/applications`)
            .then((response) => {
                        ...
                        applications.map((application, index) => (
                            <div key={index} className='application-item'>
                                ...
                                <div>
                                    ...
                                    <p><b>Status: </b> <span className={` status ${application.status === 'PENDING' ? 'pending-status' : application.status === 'ACCEPTED' ? 'accept-status' : 'denied-status'}`}>{application.status}</span></p>
                                    <button className='btn-view-detail-application' onClick={() => handleViewDetail(application)}>View detail</button>
                                </div>
                            </div>
                        ))
                    ) : (
