...
import { faClipboardUser, faMagnifyingGlass, faMoneyBillTransfer, faRightFromBracket, faUser, faUserGraduate } from '@fortawesome/free-solid-svg-icons';
import { getMentorsAdmin, getStaffsAdmin, getTransactionAdmin, getUsersAdmin, logout } from '../../services/service';
                    ...
                    <TableBody>
                        {mentors.map((mentor) => (
                            <StyledTableRow key={mentor.id}>
                                ...
                                <StyledTableCell align="center">{mentor.phoneNumber}</StyledTableCell>
                                <StyledTableCell align="center">
                                    <img className='pro-pic' src={mentor.profilePic} alt={mentor.fullName} />
                                </StyledTableCell>
                                <StyledTableCell align="center">{mentor.jobTitle}</StyledTableCell>
                                <StyledTableCell align="center">{mentor.company}</StyledTableCell>
                                <StyledTableCell align="center">{mentor.bio}</StyledTableCell>
                                <StyledTableCell align="center">{mentor.createdDate}</StyledTableCell>
                            </StyledTableRow>
                        ))}
                    </TableBody>
                </Table>
...
const TransactionManagement = () => {
    ...
    const fetchTransactionsAPI = () => {
        getTransactionAdmin().then((response) => {
                    ...
                    <TableBody>
                        {staffs.map((staff) => (
                            <StyledTableRow key={staff.id}>
                                ...
                                <StyledTableCell align="center">{staff.userName}</StyledTableCell>
                                <StyledTableCell align="center">{staff.phoneNumber}</StyledTableCell>
                                <StyledTableCell align="center">{staff.passAutoGenerate}</StyledTableCell>
    ...
    return (
        <div className="admin-management">
            <div className="sidebar">
                ...
                >
                    <FontAwesomeIcon className='font-awesome-icon' icon={faUser} /> User Management
                </div>
                <div
                    className={activeTab === 'mentor' ? 'active' : ''}
                    onClick={() => setActiveTab('mentor')}
                >
                    <FontAwesomeIcon className='font-awesome-icon' icon={faUserGraduate} /> Mentor Management
                </div>
                <div
                    className={activeTab === 'transaction' ? 'active' : ''}
                    onClick={() => setActiveTab('transaction')}
                >
                    <FontAwesomeIcon className='font-awesome-icon' icon={faMoneyBillTransfer} /> Transaction Management
                </div>
                <div
                    className={activeTab === 'staff' ? 'active' : ''}
                    onClick={() => setActiveTab('staff')}
                >
                    <FontAwesomeIcon className='font-awesome-icon' icon={faClipboardUser} />  Staff Management
                </div>
                <div className='logout-btn' onClick={() => { handleLogout() }}>Logout <FontAwesomeIcon icon={faRightFromBracket} /></div>
            </div>
