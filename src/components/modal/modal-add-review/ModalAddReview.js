...
export default function ModalAddReview({ mentorId, onClose }) {
    ...
    const handleSaveFeedback = () => {
        axiosInstance.post(`${RYI_URL}/Feedback/send-feedback`, data)
            .then((res) => {
    ...
    return (
        <Modal show onHide={onClose}>
            ...
            <Modal.Body>
                ...
                {errors.Rating && <small className='error-message'>{errors.Rating[0]}</small>}
            </Modal.Body>
            <Modal.Footer>
                <Button variant="secondary" onClick={onClose}>Close</Button>
                <Button variant="primary" onClick={handleSaveFeedback}>Save</Button>
            </Modal.Footer>
        </Modal>
    );
}
